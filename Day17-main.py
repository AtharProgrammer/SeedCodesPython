...
class Management:
    ...
    def DeleteBook(self):
        ...
        try:
            ...
            if status == True:
                connection = pyodbc.connect('DRIVER={SQL Server};SERVER='+self.server+';DATABASE='+self.database+';Trusted_Connection=yes;')

                curs = connection.cursor()

                curs.execute("select * from borrow_record where book_id=?", row[0])
                var = curs.fetchall()

                if len(var) != 0:
                    messagebox.showwarning("Critical Warning!", "You can't delete this book record")
                else:
                    curs.execute("delete from book_list where book_id=?",
                    (
                        row[0]
                    ))
    ...
    def ReturningBook(self):
        ...
        try:
            ...
            if status == True:
                connection = pyodbc.connect('DRIVER={SQL Server};SERVER='+self.server+';DATABASE='+self.database+';Trusted_Connection=yes;')
                curs = connection.cursor()
                curs.execute("delete from borrow_record where book_id=?",
                (
                    row[0]
                ))
                curs.execute("select * from book_list where book_id=?", row[0])
                var = curs.fetchone()

                book_count = var[5]
                book_count += 1

                curs.execute("update book_list set qty=? where book_id=?", (book_count, row[0]))
    ...
    def BorrowBookAgain(self, row):
        try:
            connection = conn = pyodbc.connect('DRIVER={SQL Server};SERVER='+self.server+';DATABASE='+self.database+';Trusted_Connection=yes;')

            curs = connection.cursor()
            curs.execute("update borrow_record set return_date=? where stu_roll=? and book_id=?",
            (
    ...
    def SubmitforUpdateBook(self, row):
        try:
            connection = pyodbc.connect('DRIVER={SQL Server};SERVER='+self.server+';DATABASE='+self.database+';Trusted_Connection=yes;')
            curs = connection.cursor()
            curs.execute("update book_list set book_name=?,author=?,edition=?,price=?,qty=? where book_id=?",
            (
    ...
    def ShowRecordsforReturn(self):
        if self.roll_no_entry.get() == "":
            ...
            try:
                connection = pyodbc.connect('DRIVER={SQL Server};SERVER='+self.server+';DATABASE='+self.database+';Trusted_Connection=yes;')
                curs = connection.cursor()
                curs.execute("select * from borrow_record where stu_roll=?", self.roll_no_entry.get())
                rows=curs.fetchall()
    ...
    def SearchBook(self):
        if self.book_entry.get() == "":
            ...
            try:
                connection = pyodbc.connect('DRIVER={SQL Server};SERVER='+self.server+';DATABASE='+self.database+';Trusted_Connection=yes;')
                curs = connection.cursor()
                curs.execute("select * from book_list where book_name like ?", ("%" + self.book_entry.get() + "%"))
                rows=curs.fetchall()
                if rows == None:
                    messagebox.showinfo("Database Empty","There is no data to show",parent=self.window)
                    connection.close()
    ...
    #             curs = connection.cursor()
    #             curs.execute("select * from book_list where book_id=?", y[0])
    #             row=curs.fetchone()
    ...
    def Submit(self):
        if self.id_entry.get() == "" or self.bookname_entry.get() == "" or self.author_entry.get() == "" or self.edition_entry.get() == "" or self.price_entry.get() == "" or self.qty_entry.get() == "":
            ...
            try:
                connection = pyodbc.connect('DRIVER={SQL Server};SERVER='+self.server+';DATABASE='+self.database+';Trusted_Connection=yes;')
                curs = connection.cursor()
                curs.execute("select * from book_list where book_id=?", self.id_entry.get())
                row=curs.fetchone()

                if row!=None:
                    messagebox.showerror("Error!","This book id is already exists, please try again with another one",parent=self.window)
                else:
                    curs.execute("insert into book_list (book_id,book_name,author,edition,price,qty) values(?,?,?,?,?,?)",
                                        (
                                            self.id_entry.get(),
                                            self.bookname_entry.get(),
                                            self.author_entry.get(),
    ...
    def Submit_borrow_data(self):
        if self.book_id_entry.get()=="" or self.book_name_entry.get()=="" or self.stu_roll_entry.get()=="" or self.stu_name_entry.get()=="" or self.issue_date_entry.get()=="" or self.return_date_entry.get=="" or self.course_entry.get()=="" or self.subject_entry.get()=="":
            ...
            try:
                connection = pyodbc.connect('DRIVER={SQL Server};SERVER='+self.server+';DATABASE='+self.database+';Trusted_Connection=yes;')
                curs = connection.cursor()
                curs.execute("select * from book_list where book_id=?", self.book_id_entry.get())
                row=curs.fetchone()
                book_quantity = row[5]
                if row==None:
                    ...
                    curs.execute("select * from borrow_record where stu_roll=? and book_id=?", (self.stu_roll_entry.get(),self.book_id_entry.get()))
                    row=curs.fetchone()
                    if row != None:
                        messagebox.showerror("Error!", "This book is already taken by the student")
                        connection.close()
                    else:
                        curs.execute("select * from borrow_record where stu_roll=?", self.stu_roll_entry.get())
                        rows = curs.fetchall()
                        if len(rows) >= 3:
                            messagebox.showerror("Max. 3 books to each student", "Borrow books limit has exceed")
                            connection.close()
                        else:
                            curs.execute("insert into borrow_record (book_id,book_name,stu_roll,stu_name,course,subject,issue_date,return_date) values(?,?,?,?,?,?,?,?)",
                                                (
                            ...
                            book_quantity -= 1
                            curs.execute("update book_list set qty=? where book_id=?", (book_quantity,self.book_id_entry.get()))
                            connection.commit()
                            connection.close()
                            messagebox.showinfo('Done!', "The data has been submitted")
