...
Check for missing values in the dataset and handle them appropriately (if any).
Convert the 'Date' column to a datetime format.
Extract the month and year from the 'Date' column and add them as new columns.
Calculate the total sales amount (price * quantity) for each entry and add it as a new column.
...
# Calculate the total sales amount for each entry and add it as a new column
...
# Calculate the average sales amount for each product category
category_avg_sales = df.groupby('Category')['Total Sales'].mean()
print("\nAverage sales amount for each product category:")
print(category_avg_sales)

# Find the top selling products
top_selling_products = df.groupby('Product')['Total Sales'].sum().nlargest(5)
